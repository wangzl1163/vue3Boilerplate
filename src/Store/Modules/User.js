import { SET_USER_NAME, SET_USER_AVATAR, SET_ROLES, SET_PERMISSIONS, SET_AUTH_TYPE, SET_USER_INFO } from '../MutationTypes'
...
const user = {
      ...
      GetUserInfo({ commit, state }) {
         return new Promise((resolve, reject) => {
            getUserInfo().then(res => {
               const avatar = !res.message.avatar ? state.avatar : res.message.avatar
               ...
               commit(SET_USER_NAME, res.message.user_info ? res.message.user_info.alias_name : res.message.Username)
   ...
   getters: {
      ...
      isShowProfile: state => state.authType !== 'union_auth' && state.name !== 'admin',
      userInfo: state => ({
         ...state.userInfo,
