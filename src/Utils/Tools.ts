...
const ArrayTools = {
    ...
    * @returns 所在数组中的下标
    */
   getIndex: function (array: unknown[], targetValue: unknown, key = "id") {
      return array.findIndex((item) =>
         typeof item === "object"
            ? (item as Record<string, unknown>)[key] === targetValue
            : item === targetValue
      );
   },
   /**
    ...
    * @returns 返回一个新数组
    */
   distinct: function (array: unknown[]) {
...
const StringTools = {
    ...
    * @returns 填充后的字符串
    */
   padLeft: function (str: string | number, length = 2, chars = "0") {
      const tmp = typeof str === "string" ? str : str.toString();
      return new Array(length - tmp.length + 1).join(chars) + tmp;
   },
    ...
    * @returns 填充后的字符串
    */
   padRight: function (str: string | number, length = 2, chars = "0") {
      const tmp = typeof str === "string" ? str : str.toString();
      return tmp + new Array(length - tmp.length + 1).join(chars);
   }
};
...
const ObjectTools = {
    ...
    */
   isEmpty: function (source: unknown) {
    ...
    */
   isEmptyObject: function (obj: unknown) {
      if (
...
const DateTools = {
    ...
    */
   dateTimeFormat: function (fmt: string, datetime: Date | string | number) {
    ...
    * @param {String|Date} date Date对象或日期时间字符串
    * @returns 格式化后的全日期时间字符串
    */
   sampleFormat: function (datetime: Date | string | number, separator = "-") {
      ...
      const formatNumber = (n: number) => (n.toString()[1] ? n : "0" + n);
    ...
    */
   dateDiff: function (date1: string | Date, date2: string | Date) {
      date1 =
         typeof date1 === "string"
            ? date1.includes(":")
               ? date1
...
const UrlTools = {
    ...
    * @returns 解析后的参数对象
    */
   parseQueryString: function (queryString: string) {
      if (queryString) {
...
const NumberTools = {
    ...
    * @returns 转换的数字
    */
   tryParseInt: function (str: string, def = 0, radix = 10) {
    ...
    * @returns 转换的浮点数
    */
   tryParseFloat: function (str: string, def = 0) {
    ...
    * @param {Number} num
    */
   thousandBit: function (num: number) {
...
const FunctionTools = {
    ...
    */
   debounce: function (func: (...argList: unknown[]) => unknown, wait = 500) {
    ...
    */
   throttle: function (
      func: (...argList: unknown[]) => unknown,
      wait = 500,
      execFirstCall = false
   ) {
      let timeout: number | null = null;
      let lastArgs: unknown[] | null;
      let firstCallTimestamp: number | null;
