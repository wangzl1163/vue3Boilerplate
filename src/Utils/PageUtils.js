...
const createOptions = (
   ...
   return {
      title: {text: title.text, left: 40, textStyle: { color: '#CCE6FF', fontFamily: 'Microsoft YaHei', fontSize: 14, fontWeight: 400, ...title.textStyle }},
      ...configuration,
      color: color,
      series: [
         {
...
const createPieOptions = (
   ...
   return createOptions('pie', name, data, {}, color, { config, series, seriesItemConfig: seriesItemConfiguration })
}
...
const createLineOptions = (
   ...
   return createOptions('line', name, data, echartsTitle, color, { config: configuration, seriesItemConfig: {
      symbol: 'none',
      ...seriesItemConfig
   } })
}
...
const createBarOptions = (
   ...
   return createOptions('bar', name, data, echartsTitle, color, { config: configuration, seriesItemConfig })
}
 ...
 */
const resolveAllSettledRes = (res, key, defaultValue = []) => res.map(item => {
   if (item.status === "fulfilled") {
      if (key) {
         return item.value.message[key] || item.value
      }

      return item.value.message === undefined ? item.value : item.value.message
   }
 ...
 */
const byte2GB = (bytes, precision = 2) => isNaN(bytes) ? bytes : (bytes * 1.0 / 1024 / 1024 / 1024).toFixed(precision)
 ...
 * @param {Number} precision 小数位精确度
 */
const byte2KB = (bytes, precision = 2) => isNaN(bytes) ? bytes : (bytes * 1.0 / 1024).toFixed(precision)
 ...
 * @param {Number} precision 小数位精确度
 */
const byte2MB = (bytes, precision = 2) => isNaN(bytes) ? bytes : (bytes * 1.0 / 1024 / 1024).toFixed(precision)
...
const createMultiLineOptions = (
   ...
   return createMultiOptions('line', echartsTitle, data, color, { config: configuration, seriesItemConfig: {
      symbol: 'none',
      ...seriesItemConfig
   } })
}
 ...
 */
const timestamp2DateTime = (arr,rate = 1,index = 0, formatStr = 'yyyy-MM-dd HH:mm:ss') => {
   const convert2Timestamp = (time) => time.toString().includes('.') ? time * 1000 * rate : time * rate

   arr.forEach(item => {
      if (Array.isArray(item)) {
         item[index] = DateTools.dateTimeFormat(formatStr, convert2Timestamp(item[index]))
      } else {
 ...
 */
const getColors = (num = 1) => ['#1781EB', '#00D4A2', '#A84FD5', '#F9B939', '#3C72DB', '#44B034', '#00D4FF', '#EB6D00', '#00E6B0', '#F9B939', '#C161F2'].slice(0, num)
...
const convertUnit = (arr, unit) => {
   const convert = {
      ...
      MB: byte2MB,
      GB: byte2GB,
      kbps: (speedValue, precision = 2) => isNaN(speedValue) ? speedValue : (speedValue * 1.0 / 1000).toFixed(precision)
   }
